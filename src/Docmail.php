...
use GoSoftware\Docmail\DocmailAPI as DocmailAPI;
...
class Docmail {
    ...
    public static function checkBalance($data = [], $options = []) {
        ...
        if ($balance < Config::get('docmail.MinimumBalance')) {
            $balanceIsOK = false;
            \Mail::send('docmail::alert-email', ["currentBalance" => $balance, "minimumBalance" => Config::get('docmail.MinimumBalance')], function($message)
            {
